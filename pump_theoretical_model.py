# -*- coding: utf-8 -*-
"""
Created on Mon Dec 14 18:59:56 2020
高压泵的理论模型 theoretical
console1
@author: 65404
"""
...
def initial_process(dic, tp):
    ...
    dt = min(t_p)  
    t_f = [tmp_count[i]['mean_life'] - dic[i].loc[dic[i].index[-1], 't1'] for i in idx_using]
    dtf = min(t_f)
    if dt < dtf:
        flag = 2
    else:
...
def low_pm(dic, tp, tmp_count, low_change):
    ...
    for idx_u in idx_using:
        ...
        dic[idx_u].loc[x+1, 'life'] = dic[idx_u].loc[x, 'life'] #先假定不变，然后再改正
        dic[idx_u].loc[x, 'n_pm']  = 0
        dic[idx_u].loc[x, 'n_cm'] = n_cm_func(dic[idx_u].loc[x, 't1'], dic[idx_u].loc[x, 't2'], tmp_count[idx_u]['mean_life'])
        dic[idx_u].loc[x, 'c_pm'] = 0
        dic[idx_u].loc[x, 'c_cm'] = cost_cm(idx_u, tmp_count) * dic[idx_u].loc[x, 'n_cm']
        dic[idx_u].loc[x, 'c_stop'] = 0
        dic[idx_u].loc[x, 'now'] = dic['t_now_list'][-1]
...
def high_fault(dic, tmp_count):
    ...
    for idx_f in idx_fault:
        ...
        dic[idx_f].loc[x-1, 'sum_cost'] += dic[idx_f].loc[x-1, 'c_stop']*dic[idx_f].loc[x-1, 'n_cm']   
    return dic
...
def high2low(dic, tmp_count, t_next_end, high_change, num):
    ...
    for idx_u in idx_using:
        ...
        dic[idx_u].loc[x, 'c_pm'] = 0
        dic[idx_u].loc[x, 'c_cm'] = cost_cm(idx_u, tmp_count)*dic[idx_u].loc[x, 'n_cm'] 
        dic[idx_u].loc[x, 'c_stop'] = cost_stop(idx_u, tmp_count) * dic[idx_u].loc[x, 'n_cm']
        dic[idx_u].loc[x, 'now'] = dic['t_now_list'][-1]
        dic[idx_u].loc[x, 'sum_cost'] = dic[idx_u].loc[x, 'c_cm'] + dic[idx_u].loc[x, 'c_stop']    
    if high_change == 1:
...
plt.title('高压泵多台设备理论结果2'+str(high_change))
plt.xlabel('维修周期/d')
plt.ylabel('花费/万元')
plt.show()


np.save('高压泵多台设备理论结果2'+str(high_change)+'.npy',all_record)
